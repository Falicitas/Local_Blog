...
首先，对于多项式 $f(x)$ ，若 $\ln{f(x)}$ 存在，则其必须满足：
...
多项式的求导，积分时间复杂度为 $O(n)$ ，求逆时间复杂度为 $O(n\log{n})$ ，故多项式求 $\ln$ 时间复杂度 $O(n\log{n})$ 。对数的常数项为0。
...
利用$f^k(x) = e^{k\ln f(x)}$来求解。

这会遇到一个问题：若常数项不为1时不能使用该式子。

这里运用一个trick：左移原多项式至常数项非0，且多项式系数同除以常数项的值（以保证常数项为1）。最后Exp运算后再右移，乘上对应的权值。具体的，令从左到右第一个不为零的项为$id$，则有$f^k(x) = f_{id}^k*x^{k*id}*e^{k\ln\frac{f(x)}{f_{id}*x^{id}}}$。
