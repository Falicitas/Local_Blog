...
倍增NTT，适用于解决一类带卷积的dp方程。

其核心是考虑将dp方程的状态表示成多项式的形式，并考虑**两个状态的合并**，从而实现在可接受的时间复杂度内完成程序运行。

设$dp(i,j)$为操作$i$次状态$j$的方案数，要求操作$n$次后对应的状态方案数。有诸如$dp(i,j) = \sum\limits_{k=0}^{j} dp(i-1,k) * g(j-k)$的式子，此时就可用倍增NTT来加速规划过程：
...
~~套路~~考虑两个状态的合并：有$dp(2n,j) = \sum\limits_{k=0}^{j}dp(n,k)*dp(n,j-k) + \sum\limits_{k=0}^{j-1}dp(n-1,k)*dp(n-1,j-1-k)$

由于$dp(2n) = dp(n) * dp(n) + x * dp(n-1) * dp(n-1)$
...
个人觉得将状态设计成$f_p(a,n)$表示范围$[1,a]$中取$n$个数，且末尾为$p$数（p=1时为奇数）的出发点有：
