...
对于$f^{R}\left(x\right)\equiv Q^{R}\left(x\right)g^{R}\left(x\right)\pmod{x^{n-m+1}}$，$f^R$为$f$关于$\bmod x^{n}$下的倒置，$g^R$为$g$关于$\bmod x^{m}$下的倒置；在取$g^R$的逆时是在模$x^{n-m+1}$下取逆的，最后记得将$Q^R$在$\bmod x^{n-m+1}$下倒置即可。